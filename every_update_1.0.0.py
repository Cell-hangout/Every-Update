...
class PackageManagerGUI:
    ...
    def export_packages(self):
        ...
        # --- Automatic filename with timestamp and .packagelist extension ---
        now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M")
        default_filename = f"packages_{now}.packagelist"
        file = filedialog.asksaveasfilename(
            initialfile=default_filename,
            defaultextension=".packagelist",
            filetypes=[("Package List", "*.packagelist"), ("All Files", "*.*")]
        )
        if file:
